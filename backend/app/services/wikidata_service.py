...
class WikidataService:
    ...
    def _parse_single_item(self, item:  Dict, continent: str) -> Optional[WikidataEvent]: 
        try:
            ...
            if not name or name.startswith("Q"): return None
            date_str = item.get("start", {}).get("value", "")
            year_start = parse_wikidata_year(date_str)
            if year_start is None: return None
            ...
            return WikidataEvent(
                name=str(name), description=str(description), content=content or description,
                year_start=year_start, year_end=year_start, latitude=coords[0], longitude=coords[1],
                continent=continent, period=calculate_period(year_start)
            )
        except Exception:
            return None